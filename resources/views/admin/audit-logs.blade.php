<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-amber-700">Security workspace</p>
    ...
    <div class="min-h-screen bg-stone-50 py-8">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <div class="grid gap-4 sm:grid-cols-3">
                @foreach ([['label' => 'Total events', 'value' => $summary['total']], ['label' => 'Events today', 'value' => $summary['today']], ['label' => 'Active actors', 'value' => $summary['actors']] ] as $metric)
                    <div
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-sm text-stone-500">Security and operational actions recorded by HotelHub.</p>
                </div>
                <form method="get" class="mt-5 flex flex-col gap-3 sm:flex-row">
                    <select
                        name="action"
                        class="rounded-lg border-stone-300 sm:max-w-sm"
                    >
            ...
            >
                <div class="border-b border-stone-200 p-6">
                    <h3 class="text-xl font-semibold">Activity stream</h3>
                    <p class="mt-1 text-sm text-stone-500">{{ $logs->total() }} events matching this view</p>
                </div>
                    ...
                    <table class="w-full text-left text-sm">
                        <thead>
                            ...
                            >
                                ...
                                <th class="px-6 py-4">New values</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($logs as $log)
                                ...
                                >
                                    <td
                                        class="whitespace-nowrap px-6 py-4 text-stone-600"
                                    >
                                        {{ optional($log->created_at)->format('M d, Y H:i') }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800"
                                            >{{ $log->action }}</span
                                        >
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td
                                        class="max-w-xs truncate px-6 py-4 text-xs text-stone-500"
                                        title="{{ json_encode($log->new_values) }}"
                                    >
