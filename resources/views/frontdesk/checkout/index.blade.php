<x-app-layout>
    ...
    <div class="bg-stone-50 py-8">
        <div class="mx-auto max-w-5xl space-y-4 px-4 sm:px-6 lg:px-8">
            <p class="text-sm text-stone-500">Today's checked-in departures. Room and posted guest-service charges must be paid before checkout.</p>
                ...
                >
                    ...
                    >
                        <div>
                            ...
                            <p class="mt-1 text-sm text-stone-500">Room {{ $reservation->room->room_number }} · {{ $reservation->room->roomType->name }}</p>
                            <div class="mt-3 grid grid-cols-4 gap-4 text-sm">
                                <div>
                                    <p class="text-xs uppercase tracking-widest text-stone-400">Room</p>
                                    <p class="font-semibold">${{ number_format($reservation->total, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs uppercase tracking-widest text-stone-400">Services</p>
                                    <p class="font-semibold">${{ number_format($folio, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs uppercase tracking-widest text-stone-400">Paid</p>
                                    <p class="font-semibold">${{ number_format($paid, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs uppercase tracking-widest text-stone-400">Balance</p>
                                    <p
                                        class="font-semibold {{ $balance > 0 ? 'text-rose-700' : 'text-emerald-700' }}"
                        ...
                        @if ($balance <= 0)
                            <form
                                ...
                                onsubmit="
                                    return confirm(
                                        'Complete checkout and send the room to housekeeping?',
                                    );
                                "
                            >
                                @csrf
                        ...
                        @else
                            <a
                                href="{{ route('frontdesk.payments.index', ['search' => $reservation->reservation_number]) }}"
                                class="rounded-lg border border-stone-300 px-4 py-2 text-sm font-semibold text-stone-700"
                                >Collect payment</a
                            >
                        @endif
                    </div>
            ...
            @empty
                ...
                >
                    No checked-in departures scheduled for today.
                </div>
            @endforelse
        </div>
    </div>
