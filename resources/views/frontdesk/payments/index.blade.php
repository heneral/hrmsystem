<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mt-1 text-2xl font-semibold text-stone-900">Payments</h2>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            >
                <div class="border-b border-stone-200 p-5">
                    <h3 class="font-semibold">Outstanding balances</h3>
                    <p class="mt-1 text-sm text-stone-500">Record permitted payments; completed transactions cannot be edited here.</p>
                </div>
                            ...
                            <div class="p-5">
                                ...
                                >
                                    <div>
                                        ...
                                        <p class="mt-2 text-sm">Balance: <span class="font-semibold">${{ number_format((float) $reservation->total + $folio - $paid, 2) }}</span></p>
                                    </div>
                                    <form
                                        method="post"
                                        action="{{ route('frontdesk.payments.store', $reservation) }}"
                                    ...
                                    >
                                        ...
                                        <input
                                            ...
                                            min="0.01"
                                            step="0.01"
                                            max="{{ (float) $reservation->total + $folio - $paid }}"
                                            required
                                            placeholder="Amount"
                                            class="w-28 rounded-lg border-stone-300 text-sm"
                                        /><select
                                            name="payment_method"
            ...
            >
                <div class="border-b border-stone-200 p-5">
                    <h3 class="font-semibold">Recent payment activity</h3>
                </div>
                <div class="divide-y divide-stone-100">
                    @forelse ($payments as $payment)
                        <div class="flex justify-between gap-4 p-4 text-sm">
                            <span
                                >{{ $payment->reservation?->reservation_number ?? 'Reservation' }}</span
                            ><span class="font-semibold"
                                >${{ number_format($payment->amount, 2) }} · {{ $payment->status->value }}</span
                            >
                        </div>
                    @empty
